...
*/
   ...
   function recuperarInformacionTaller($id){
    ...
    //regresa Talleres en los que está inscrita la alumna con id $id
    ...
    //regresa Talleres que están disponibles para la alumna con id $id
    ...
    //regresa la información del Taller con id $id
    function infoTaller($id,$inscripcion) {
        ...
        if ($inscripcion === TRUE) { // habilita botón para inscribir
        ...
        } else { //hablita botón para eliminar inscripción
            $info.= '<button class="btn red col s3" onclick="eliminarInscripcionTaller('.$id.')"> Eliminar inscripción </button>';
        }
        closeDB($mysql);
        return $info;
    }
    ...
    function alumnasInscritas($id){
        ...
        $tabla.='<thead>
                    ...
                    <th class="minWidth">Fecha de inscripción</th>
                    </tr>
                    </thead>
                    <tbody>';
...
*/
   ...
   function registrarNoticia($tit, $cont, $img, $descripcionimg,$fecha, $usuario) {
    ...
    function modificaNoticia($id, $titulo, $contenido, $img, $descripcionimagen) {
        ...
        if ($img !== NULL) { //se introdujo alguna imagen en el formulario 
            if ($imagenactual) { //hay una imagen asociada con la noticia
                if ($img != 0) {  //la imagen que se introdujo en el formulario es diferente a la que se recuperó
                    if (!(modificaImagen($id, $img, $descripcionimagen, $mysql))) { //modificar imagen
                        return false;
                    }
                } else if ($descripcionimagen != $imagenactual["descripcion"]){ //verificar si solo se cambio la descripción de la imagen recuperada
                    if (!(modificaImagen($id, $imagenactual["ubicacion"], $descripcionimagen, $mysql))) { //modificar descripción de la imagen
                        return false;
                    }
                } 
            } else {
                if (!(registrarImagen($id,$img,$descripcionimagen,$mysql))) { //no había una imagen, y agregó una nueva
                    return false;
                }
            } 
        } else if ($imagenactual){ //se eliminó la imagen de la noticia
    ...
    function vistaNoticia($id) {
        ...
        $vistaNoticia = '<h1 id="tituloNoticia">'.$noticia["titulo"].' </h1> <hr>'; //título de la noticia
        $vistaNoticia.=  '<div id="infonoticia"> Formación Integral de La Mujer &nbsp | &nbsp Querétaro, México. 
        &nbsp | &nbsp' .$noticia["fecha"]. '</div><br>'; //información de la noticia
...
*/
    ...
    function registrarHistorialAcademico($id,
        ...
        /*VISUALIZAR la información de alumnas*/
        ...
        function tablaInfoTotal(){
            ...
            $tabla.='<thead>
                    ...
                    <th class="minWidth">Calle y número </th>
                    <th class="minWidth">Colonia</th>
                    <th class="minWidth">Código Postal</th>
                    <th class="minWidth">Municipio</th>
                    <th class="minWidth">Ocupación</th>
                    <th class="minWidth">Estado Civil</th>
                    <th class="minWidth">Nacimiento</th>
                    <th class="minWidth">Género</th>
                    ...
                    <th class="minWidth">Como se enteró de FIM</th>
                    </tr>
                    </thead>
                    <tbody>';
        ...
        function tablaInfoPDF(){
            ...
            $tabla.='<thead>
                    ...
                    <th class="minWidth"><strong>Calle y número </strong></th>
                    <th class="minWidth"><strong>Colonia </strong></th>
                    <th class="minWidth"><strong>Código Postal </strong></th>
...
*/
 ...
 <img class="rs" src="'.$row["ubicacion"].'"/> </div> </div> </div> </div>';   
           ...
           $lista.='<div class="caption center-align">
                      <div class="container">
                        <div class="row center">
                          <h3 class="col s1¡2 light pinkFim-opa">!Únete a FIM!</h3>
                        </div>
                        <div class="row center">
                          <h5 class="header col  s1¡2 light flow-text pinkFim-opa">'.$row["titulo"].'</h5>
                        </div>
                      </div>
                    </div>';
            $lista.='</li>';
   ...
   function obtenerFunciones(){
       //Arreglo asociativo con el nombre del archivo para cada función.
       $nombresArchivos = [
   ...
   //Verificar que la sesión actual sea de un usuario tipo administrador
